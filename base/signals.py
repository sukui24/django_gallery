...
def image_deleter(sender, instance, **kwargs):
    image_path = os.path.join('./media/images/', instance.image.name)

    if os.path.isfile(image_path):
        os.remove(image_path)  # remove file if it exists
    else:
...
def delete_old_image(sender, instance, **kwargs):
    ...
    try:
        ...
        if old_instance.image.name != instance.image.name:  # compare to see if image changed
            image_deleter(sender=ImageModel, instance=old_instance, **kwargs)
...
def update_unique_name(sender, instance, created, **kwargs):
    if created or instance.unique_name:
        ...
        instance.__class__.objects.filter(pk=instance.pk).update(
            unique_name=filename)  # now unique_name = filename without 'user_id/'
